...
async function fetchPipelinesFromDB() {
    try {
        const response = await fetch('getData.php?table=Pipelines');
        if (!response.ok) {
...
async function fetchPointsFromDB() {
    try {
        const response = await fetch('getData.php?table=Points');
        if (!response.ok) {
            throw new Error(`Ошибка HTTP: ${response.status}`);
        }
...
async function fetchOilTransferFromDB() {
    try {
        const response = await fetch('getData.php?table=oiltransfer');
        if (!response.ok) {
            throw new Error(`Ошибка HTTP: ${response.status}`);
        }
...
fetch('getData.php?table=Points')
    .then(response => response.json())
    .then(points => {
...
fetch('getData.php?table=Reservoirs')
    .then(response => response.json())
    .then(data => {
